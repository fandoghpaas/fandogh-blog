# معرفی فندق
فندق یک پلتفرم برای تسهیل در سااده‌سازی Deployment سرویس‌هاست که به شما کمک می‌کنه هر سرویسی رو از یک docker image به
...
* **کلاینت فندق**   
کلاینت فندق یک نرم‌افزار تحت کامندلاین است که به کاربران کمک می‌کنند از فندق استفاده کنند.
* **Image**   
...
* **Image Version**  
برای مدیریت روی نسخه‌های مختلف از سرویستون شما باید به هر Imageای که در فندق منتشر می‌کنید یک شماره ورژن نسبت دهید.
* **Service**    
...
* **Namespace**    
Namespace یک دسته‌بندی برای serviceهاست، سرویس‌های یک namespace از طریق شبکه داخلی قادر به دسترسی به یک‌دیگر هستند و فضای ذخیره‌سازی اشتراکی دارند. در حال حاضر برای هر کاربر پس از عضویت به طور خودکار یک  Namespace ایجاد می‌شود.
*  **shared-storage یا فضای ذخیره‌سازی اشتراکی**      
یک فضای ذخیره‌سازی که در یک namespace تعریف شده است و تمام سرویس‌های داخل namespace به آن دسترسی دارند. این فضا به طور خودکار در مسیر `/mnt/shared-volume/` هر سرویس mount می‌شود.
